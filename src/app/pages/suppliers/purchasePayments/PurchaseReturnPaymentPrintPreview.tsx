...
import PrintablePurchaseReturnPayment from "./PrintablePurchaseReturnPayment";
import type { Settings } from "@/shared";

export default function PurchaseReturnPaymentPrintPreview() {
    const { id } = useParams();

    const { data: paymentData, isLoading } = useGetPurchaseReturnPaymentByIdQuery(id as string, {
        skip: !id,
    });
    ...
    return (
        <div className="bg-white min-h-screen py-10 print:bg-white print:py-0">
            <div className="max-w-[850px] mx-auto mb-6 px-4 sm:px-0 print:hidden">
                <Link to={`/dashboard/purchase-returns/payments/${id}`}>
                    <Button variant="outline" className="gap-2">
                        <ArrowLeft className="h-4 w-4" /> Back to Details
                    </Button>
                </Link>
            </div>
            <PrintablePurchaseReturnPayment
