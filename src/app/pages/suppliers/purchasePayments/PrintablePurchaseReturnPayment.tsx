...
export default function PrintablePurchaseReturnPayment({ payment, to }: Props) {
    ...
    const supplier = payment?.purchase_return?.supplier;
            ...
            <div id="payment-receipt" className="invoice-box print:border-0 print:p-0">
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                    <div className="border border-gray-300">
                        <div className="bg-gray-100 px-3 py-1 font-bold details-text border-b border-gray-300">Refund From (Supplier)</div>
                        <div className="p-3 details-text min-h-[80px]">
                            <p className="font-bold">{supplier?.name}</p>
                            <p className="whitespace-pre-line">{(supplier as any)?.address}</p>
                            {supplier?.phone && <p>T: {supplier.phone}</p>}
                            {supplier?.email && <p>E: {supplier.email}</p>}
                        </div>
                    </div>
                    <div className="border border-gray-300">
                        <div className="bg-gray-100 px-3 py-1 font-bold details-text border-b border-gray-300">Refund To (Company)</div>
                        <div className="p-3 details-text min-h-[80px]">
                ...
                <div className="w-full mb-6 text-sm">
                    <table className="w-full border-collapse table-border">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <div className="font-bold">Refund for Purchase Return #{payment?.purchase_return?.return_number || payment?.purchase_return?.id}</div>
                                    {payment?.invoice && (
                ...
                <div className="mt-12 grid grid-cols-2 gap-8 details-text">
                    <div className="text-center">
                        ...
                        <p>Supplier Authorization</p>
                    </div>
                    <div className="text-center">
                        <div className="border-b border-gray-400 h-10 w-48 mx-auto mb-1"></div>
                        <p>Company Acknowledgement</p>
                    </div>
                </div>
