...
export default function PrintableInvoice({ invoice, from, to }: Props) {
  ...
  // Calculations
  const subtotal = Number(invoice?.order?.totalAmount || 0);
  const discount = Number(invoice?.order?.discountAmount || 0);
  const gstRate = 0.06; // Based on PDF showing 6%
  const gstAmount = Number(invoice?.order?.taxAmount || (subtotal - discount) * gstRate);
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              <tr className="text-center">
                ...
                <td className="border border-gray-300 p-1">{invoice?.order?.deliveryDate ? formatDate(String(invoice.order.deliveryDate)) : "-"}</td>
                <td className="border border-gray-300 p-1">-</td>
          ...
          <table className="w-full table-text border-collapse">
            ...
            <tbody>
              {invoice?.order?.items?.map((item, index) => {
                ...
                const tax = Number(item.taxAmount) || (pretax * (taxRate / 100));
